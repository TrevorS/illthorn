import { type GameTag, makeTag, normalizeTagName, TagState } from "./tag";
import { parseAttrs } from "./tag/attributes";
import type { TagName } from "./tag/names";

export const Busy = Symbol("busy");
export const Ok = Symbol("ok");

type ParserCursor = { idx: number };

type ConsumeOpts = { delimiter: string; inclusive: boolean };
...
export class Parser {
  ...
  parse(text: string) {
    ...
    while (cursor.idx < text.length) {
      ...
      char === "<" ? this.handleTagStart(cursor, text) : this.handleTextNode(cursor, text);
  ...
  reset() {
    this.pending.length = this.done.length = 0;
  }
  ...
  closeTag(tagInfo: string): void {
    const cursor = { idx: 0 };
    const gameName = this.consumeToChar(cursor, tagInfo, { delimiter: " ", inclusive: false });
  ...
  openTag(tagInfo: string) {
    const cursor = { idx: 0 };
    const tagName = this.consumeToChar(cursor, tagInfo, { delimiter: " ", inclusive: false });
  ...
  parseTag(tagInfo: string) {
    ...
    const _lastPending = this.pending[this.pending.length - 1];
    const _lastDone = this.done[this.done.length - 1];
    ...
    */
    ...
    isClosure ? this.closeTag(tagInfo) : this.openTag(tagInfo);
  }
}
