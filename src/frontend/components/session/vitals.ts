...
export class Vitals extends HTMLElement {
  ...
  render() {
    this.append(this.health, this.stamina, this.spirit, this.mana, this.mind, this.stance, this.encum);
  }

  bindBarEvent(feedId: string, bar: ProgressBar) {
    this.session.bus.subscribeEvent<GameTag>(`metadata/progressBar/${feedId}`, ({ detail: feedInfo }) => this.updateState(bar, feedInfo));
  }
