type HilitesProfile = { patterns: Record<string, string>; groups: Record<string, Record<string, string>> };
...
export function getSheet() {
  const styleEle = document.querySelector<HTMLStyleElement>("#hilites") || makeStylesheet();
...
export async function reloadHilites() {
  ...
  Object.entries(settings.groups).forEach(([group, rules]) => insertGroupRule(group, rules));

  _CACHE.length = 0;

  Object.entries(settings.patterns).forEach(([pattern, color]) => _CACHE.push([deserializeHilites(pattern), color]));
...
export function deserializeHilites(hilite: string) {
  if (isHiliteRegex(hilite)) return new RegExp(hilite.slice(1, hilite.length - 2), "g");
  return new RegExp(hilite, "g");
}

export function serializeHilites(hilite: string) {
  return (isHiliteRegex(hilite) ? new RegExp(hilite.slice(1, hilite.length - 2), "g") : new RegExp(hilite, "g")).toString();
}

export function addPattern(_group: string, _pattern: string) {}

export function addGroup(_group: string, _rules: Record<string, string>) {}

export function remotePattern(_pattern: string) {}

export function removeGroup(_group: string) {}
