...
export async function detectLichSessions(): Promise<Illthorn.LichSessionDescriptor[]> {
  ...
  const pendingSessions = (await fs.readdir(scriptingSessionDir)).map(
    async (file: string) => await json.read<Illthorn.LichSessionDescriptor>(path.join(scriptingSessionDir, file)),
  );
